...
//随机文字烟花内容
const randomWords = ["小常子\n生日快乐哇!\n"];
const wordDotsMap = {};
randomWords.forEach((word) => {
const screenWidth = window.innerWidth;
const fontSize = Math.min(90, screenWidth / 10) + 'px'; // 最大90px，但不超过屏幕宽度的1/10
wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀", fontSize);
});
...
const soundManager = {
    ...
    sources: {
        lift: {
            ...
            fileNames: ["audio_lift1.mp3", "audio_lift2.mp3", "audio_lift3.mp3"]
        },
        burst: {
            volume: 1,
            playbackRateMin: 0.8,
            playbackRateMax: 0.9,
            fileNames: ["audio_burst1.mp3", "audio_burst2.mp3"]
        },
        burstSmall: {
            volume: 0.25,
            playbackRateMin: 0.8,
            playbackRateMax: 1,
            fileNames: ["audio_burst-sm-1.mp3", "audio_burst-sm-2.mp3"]
        },
        crackle: {
            volume: 0.2,
            playbackRateMin: 1,
            playbackRateMax: 1,
            fileNames: ["audio_crackle1.mp3"]
        },
        crackleSmall: {
            volume: 0.3,
            playbackRateMin: 1,
            playbackRateMax: 1,
            fileNames: ["audio_crackle-sm-1.mp3"]
        }
    },
    preload() {
        // 保持原有 preload 代码不变
